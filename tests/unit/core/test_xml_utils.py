...
def test_serialize_xml() -> None:
    ...
    xml_default = serialize_xml(root)
    assert "Robot: test" in xml_default
    assert "Generated by: LinkForge\n" in xml_default
    assert "v" not in xml_default.split("Generated by: LinkForge")[1].split("\n")[0]
    assert "Date:" not in xml_default
