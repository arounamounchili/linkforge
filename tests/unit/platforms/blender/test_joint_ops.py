...
def test_joint_ops_create_joint(mock_context, scene) -> None:
    ...
    # Test Poll
    assert bpy.ops.linkforge.create_joint.poll() is True  # type: ignore[attr-defined]
...
def test_joint_ops_delete_joint(mock_context, scene) -> None:
    ...
    # Execute delete
    bpy.ops.linkforge.delete_joint()  # type: ignore[attr-defined]
    assert joint_name not in bpy.data.objects
    assert len(safe_get_linkforge_scene(scene).ros2_control_joints) == 0
...
def test_joint_ops_auto_detect(mock_context, scene) -> None:
    ...
    assert child_link_mesh is not None
    ...
    assert parent_link_mesh is not None
    ...
    # Auto-detect
    bpy.ops.linkforge.auto_detect_parent_child()  # type: ignore[attr-defined]
...
def test_joint_ops_robustness_no_links(mock_context, scene) -> None:
    ...
    assert bpy.ops.linkforge.create_joint.poll() is False  # type: ignore[attr-defined]
